...
Generates 100% Watertight, ready-to-print Binary STL files for:
- Involute Spur Gears & Planetary Mechanisms
- Helical Milling Cutter Rollers
- Music Box Melody Pin Cylinders, Comb Reeds, and Cast Bed Plates
"""
import math
import struct
...
class PrecisionSTLBuilder:
    ...
    def create_musicbox_drum_stl(self, radius: float = 4.8, length: float = 16.0, num_pins: int = 48) -> bytes:
        """
        Creates 100% Watertight Solid STL for Melody Pin Cylinder Drum with raised micro-pins.
        """
        triangles = []
        n_pts = 36
        n_rings = 16
        ...
        for r in range(n_rings):
            ...
            for p in range(n_pts):
                ...
                n1 = self._calc_normal(p1, p2, p3)
                triangles.append((n1, p1, p2, p3))
                n2 = self._calc_normal(p1, p3, p4)
    ...
    def create_comb_reeds_stl(self, width: float = 16.0, length: float = 12.0, num_teeth: int = 18) -> bytes:
        """
        Creates 18-tooth graduated steel comb reeds solid STL.
        """
        triangles = []
    ...
    def create_involute_gear_stl(self, module: float = 1.5, teeth_count: int = 20, face_width: float = 12.0, shaft_dia: float = 6.0, tolerance: float = 0.25, cots_mount: str = "608zz") -> bytes:
        m = module
        z = teeth_count
        r_pitch = (m * z) / 2.0
        r_tip = r_pitch + (1.0 * m)
        ...
        for i in range(z):
            ...
            outer_pts.append((r_root * math.cos(t_center - tooth_thickness_angle * 1.5), r_root * math.sin(t_center - tooth_thickness_angle * 1.5)))
            outer_pts.append((r_pitch * math.cos(t_center - tooth_thickness_angle * 0.9), r_pitch * math.sin(t_center - tooth_thickness_angle * 0.9)))
            outer_pts.append((r_tip * math.cos(t_center - tooth_thickness_angle * 0.5), r_tip * math.sin(t_center - tooth_thickness_angle * 0.5)))
            outer_pts.append((r_tip * math.cos(t_center + tooth_thickness_angle * 0.5), r_tip * math.sin(t_center + tooth_thickness_angle * 0.5)))
            outer_pts.append((r_pitch * math.cos(t_center + tooth_thickness_angle * 0.9), r_pitch * math.sin(t_center + tooth_thickness_angle * 0.9)))
            outer_pts.append((r_root * math.cos(t_center + tooth_thickness_angle * 1.5), r_root * math.sin(t_center + tooth_thickness_angle * 1.5)))
        ...
        return self._pack_stl(f"EduMechanic-3D Involute Gear (m={module}, z={teeth_count})", triangles)
